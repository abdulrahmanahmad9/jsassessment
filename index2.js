const menu = [
  ...
  const updated = addMenuItem (menu,newMenuItem);
  console.log(updated);
  ...
  function countMainCourseItems(menu) {
    // Your code here
    const count = menu.filter(meu => meu.category === "Main Course").length;
    return count;
  }
  ...
  function listMenuItemNamesByCategory(menu, category) {
    ...
    const list = menu.filter(meu => meu.category === category);

    const items = list.map(meu => meu.name);
  ...
  **********/
  ...
  function getCheapestMenuItem(menu) {
    // Your code here
    const cheapestItem = menu.reduce((cheapestSoFar, currentPrice) => {

        if (!cheapestSoFar || currentPrice.price < cheapestSoFar.price) 
        
        {return currentPrice;} 
        
        return cheapestSoFar;}, 0);
  ...
  function getMenuItemsByIngredient(menu, ingredient) {
    ...
    menu.forEach ((meu) => { 
        ...
        meu.ingredients.forEach ((ingredient) =>{
